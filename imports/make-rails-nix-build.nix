# imports/make-rails-nix-build.nix
{
  ...
  # outside (including your dev-time manage-redis).
  #
  # Default `null` means "auto-detect from Gemfile.lock" — so apps whose
  # flake.nix was generated by an older template (and doesn't pass this)
  # still get the redis sidecar when their Gemfile has redis / sidekiq /
  # resque. Set `true`/`false` to force on/off.
  needsRedis ? null,
  ...
}: let
  ...
  # Auto-detect needsRedis from the app's Gemfile (so apps with older
  # template-generated flake.nix that don't pass this explicitly still get
  # the transient redis-server). Caller can force on/off by passing true/false.
  autoFrameworkInfo = (import ./detect-framework.nix) { inherit src; };
  resolvedNeedsRedis =
    if needsRedis != null then needsRedis else autoFrameworkInfo.needsRedis;
