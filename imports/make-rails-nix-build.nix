      ...
      ${
        ...
        then ''
          ...
          if [ -n "$TAILWIND_GEM_DIR" ]; then
            ...
            chmod u+wx "$TAILWIND_GEM_DIR/exe/tailwindcss-x86_64-linux" || echo "chmod failed"
            ...
            ruby -e "path = '$TAILWIND_GEM_DIR/exe/tailwindcss-x86_64-linux'; puts \"File.exist?: #{File.exist?(path)}\"; puts \"File.executable?: #{File.executable?(path)}\"; puts \"File.readable?: #{File.readable?(path)}\""
