# flake.nix - Universal Ruby Application Template
{
  ...
  outputs = {
    ...
    mkOutputsForSystem = system: let
      ...
      bundlerEnvPackage = pkgs.bundlerEnv (
        {
          name = "${framework}-gems";
          ruby = rubyPackage;
      ...
      devShells = {
        ...
        with-bundlerenv = pkgs.mkShell {
          ...
          shellHook = defaultShellHook + ''
            ...
            echo "   Mode: ${if builtins.pathExists ./gemset.nix then "gemset.nix + Gemfile.lock" else "Gemfile.lock only"}"
            ...
            '' else if frameworkInfo.isWebApp then ''
